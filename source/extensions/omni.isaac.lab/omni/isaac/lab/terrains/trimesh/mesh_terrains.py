...
def parkour_step_terrain(
    difficulty: float, cfg: mesh_terrains_cfg.MeshStepTerrainCfg
) -> tuple[list[trimesh.Trimesh], np.ndarray]:

    # resolve the terrain configuration
    step_height = cfg.step_height_range[0] + difficulty * (cfg.step_height_range[1] - cfg.step_height_range[0])
    step_length = cfg.step_length_range[1] + difficulty * (cfg.step_length_range[0] - cfg.step_length_range[1])

    # compute number of steps in x direction
    num_steps_x = (cfg.size[0] - 2 * cfg.border_width - cfg.platform_width) // (2 * cfg.step_width) + 1
    ...
    box_pos = (terrain_center[0], terrain_center[1], terrain_center[2])
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    for k in range(num_steps):
        if cfg.up:
            box_dims = (
                terrain_size[0] - 2 * k * cfg.step_width - 2 * offset,
                # terrain_size[1] / 2,
                step_length,
                (k + 2) * step_height,
            )
            box_pos = (terrain_center[0] + k * cfg.step_width + offset, terrain_center[1], terrain_center[2] + k * step_height / 2)
        else:
            box_dims = (
                terrain_size[0] - 2 * (k-1) * cfg.step_width - 3 * offset,
                # terrain_size[1] / 2,
                step_length,
                (k + 2) * step_height,
            )
            box_pos = (terrain_center[0] - (k-1) * cfg.step_width - offset, terrain_center[1], terrain_center[2] + k * step_height / 2)
        box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
        meshes_list.append(box_middle)
    # origin of the terrain
    if cfg.up:
...
def single_step_terrain(
    ...
    box_pos = (terrain_center[0] - offset, terrain_center[1] - 0.7, terrain_center[2])
...
def pit_cross_terrain(
    difficulty: float, cfg: mesh_terrains_cfg.MeshPitCrossTerrainCfg
) -> tuple[list[trimesh.Trimesh], np.ndarray]:
    ...
    box_pos = (terrain_size[0] / 7, terrain_center[1], terrain_center[2]-box_dims[2]/2)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)

    box_pos = (terrain_center[0] * 2 - terrain_size[0] / 7, terrain_center[1], terrain_center[2]-box_dims[2]/2)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (terrain_center[0], terrain_center[1], terrain_center[2] - cfg.pit_depth)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (terrain_center[0] + terrain_size[0] / 2 - 0.1, terrain_center[1], terrain_center[2] + box_dims[2] / 2)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (terrain_center[0], terrain_center[1] + terrain_size[1] / 2 - 0.1, terrain_center[2] + box_dims[2] / 2)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
...
def corridor_terrain(
    difficulty: float, cfg: mesh_terrains_cfg.MeshCorridorTerrainCfg
) -> tuple[list[trimesh.Trimesh], np.ndarray]:
    ...
    box_pos = (terrain_center[0], terrain_center[1] - 2.5, terrain_center[2])
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (terrain_center[0] + 1.0, terrain_center[1], terrain_center[2] - box_dims[2] / 2)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (1.0, terrain_center[1], terrain_center[2]-0.15)
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (terrain_center[0] + terrain_size[0] / 2 - 0.1, terrain_center[1], terrain_center[2])
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
    ...
    box_pos = (terrain_center[0], terrain_center[1] - 1.0, terrain_center[2])
    box_middle = trimesh.creation.box(box_dims, trimesh.transformations.translation_matrix(box_pos))
    meshes_list.append(box_middle)
