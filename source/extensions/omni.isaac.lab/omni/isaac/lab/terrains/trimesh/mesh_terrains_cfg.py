...
@configclass
class MeshStepTerrainCfg(SubTerrainBaseCfg):
    """Configuration for a terrain with a gap around the platform."""
    ...
    step_width: float = MISSING
    """The width of the steps (in m)."""
    step_length_range: tuple[float, float] = (0.5, 2.0)
    """The minimum and maximum length of the steps (in m)."""
    platform_width: float = 1.0
    """The width of the square platform at the center of the terrain. Defaults to 1.0."""
    holes: bool = False
    """If True, the terrain will have holes in the steps. Defaults to False."""
    up: bool = True
    """If True, the terrain will have steps that go up. Defaults to True."""

@configclass
class MeshSingleStepTerrainCfg(SubTerrainBaseCfg):
    """Configuration for a terrain with a gap around the platform."""

    function = mesh_terrains.single_step_terrain
...
@configclass
class MeshPitCrossTerrainCfg(SubTerrainBaseCfg):
    """Configuration for a terrain with a pit that leads out of the pit."""

    function = mesh_terrains.pit_cross_terrain
